...
class TrackedAuthNotifier extends StateNotifier<AuthState> implements AuthNotifier {
  ...
  Future<void> login(String username, String password) async {
    ...
    await Future.delayed(const Duration(milliseconds: 10)); // Simulate network call
...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockAnalyticsFacade.logEvent(any(), parameters: any(named: 'parameters')))
        .thenAnswer((_) async {});
    when(() => mockAnalyticsFacade.logScreenView(any(), any()))
        .thenAnswer((_) async {});
    
    // Setup error handler mocks
    when(() => mockAuthErrorHandler.processError(
      any(), 
      any(),
